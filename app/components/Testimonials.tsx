...
import { type Testimonial, testimonials } from "./Testimonials.data";
...
const STARS = Array.from({ length: MAX_STARS }, (_, i) => (
  <Star
    key={i}
    className="w-4.5 h-4.5 fill-yellow-400 text-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.3)]"
  />
));

const TestimonialCard = memo(({ testimonial }: { testimonial: Testimonial; index: number }) => (
  <div
  ...
  >
    <div className="flex flex-col gap-6">
      <div className="flex gap-1.5">
        {STARS.slice(0, testimonial.stars)}
      </div>
